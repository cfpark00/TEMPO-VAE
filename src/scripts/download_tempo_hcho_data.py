#!/usr/bin/env python3
"""Download TEMPO HCHO L2 data files from URL list."""
...
def main(config_path, overwrite=False, debug=False):
    ...
    # Convert RAD URLs to HCHO URLs
    with open(rad_file_list, 'r') as f:
        rad_urls = [line.strip() for line in f if line.strip()]

    hcho_urls = []
    file_mapping = {}  # Map HCHO files to RAD files

    for url in rad_urls:
        # Replace RAD_L1 with HCHO_L2 in both path and filename
        hcho_url = url.replace('RAD_L1', 'HCHO_L2')
        hcho_urls.append(hcho_url)

        # Track mapping
        rad_name = Path(url).name
        hcho_name = Path(hcho_url).name
        file_mapping[hcho_name] = rad_name
    ...
    # Save URL mapping
    mapping_path = output_dir / 'rad_to_hcho_mapping.yaml'
    with open(mapping_path, 'w') as f:
        yaml.dump({'file_mapping': file_mapping}, f)

    # Download
    failed = []
    for i, url in enumerate(hcho_urls, 1):
        filename = Path(url).name
        output_path = raw_dir / filename

        if output_path.exists():
            print(f"[{i}/{len(hcho_urls)}] EXISTS: {filename}")
            continue

        print(f"[{i}/{len(hcho_urls)}] Downloading: {filename}")
    ...
    print(f"\nDone. Downloaded: {len(hcho_urls) - len(failed)}, Failed: {len(failed)}")
    if failed:
        print("Failed URLs:")
        for url in failed:
            print(f"  {url}")
